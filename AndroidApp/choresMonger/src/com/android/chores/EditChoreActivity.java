...
public class EditChoreActivity extends Activity implements OnClickListener {
	...
	public void initalizeElements() {
		...
		((EditText) (findViewById(R.id.editTxtChoreTitleVal))).setText(
				chore.getName(), TextView.BufferType.EDITABLE);
		if (chore.getUsers().size() > 0)
			((EditText) (findViewById(R.id.editxtCreateChoreUserVal))).setText(
					chore.getUsers().get(0).getName(),
					TextView.BufferType.EDITABLE);
		else
			((EditText) (findViewById(R.id.editxtCreateChoreUserVal))).setText(
					"", TextView.BufferType.EDITABLE);

		((EditText) (findViewById(R.id.txviewChorePointsVal))).setText(
				Double.toString(chore.getPointValue()),
				TextView.BufferType.EDITABLE);
	}

	public void updateChore() {
		chore.setName(((EditText) findViewById(R.id.editTxtChoreTitleVal))
				.getText().toString());
		User user = UserImpl
				.getUserByName(((EditText) findViewById(R.id.editxtCreateChoreUserVal))
						.getText().toString());
		if (chore.getUsers().size() > 0)
			chore.addUser(user);
		chore.setPointValue(Double
				.parseDouble(((EditText) findViewById(R.id.txviewChorePointsVal))
						.getText().toString()));
	...
	public void deleteChore() {
		...
		UserImpl.getUser(user_id).removeChore(chore);
